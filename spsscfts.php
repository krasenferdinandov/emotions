...
for($i=0;$i<DOMAINS_NUMBER;$i++){
	...
	echo '<td><center>F'.$i.'_'.$bg_name.'</center></td>';
	//echo '<td><center>F'.$i.', '.$en_name.', '.$bg_name.'</center></td>';
...
for($i=0;$i<EMOTIONS_NUMBER;$i++){
	...
	{
		$name = $r["bg_name"];
		$domain_1 = $r["domain_id"];
		
	}
	echo '<td>F'.$domain_1.',E'.$i.','.$name.'</td>';
}
//------------------>
for($i=0;$i<THEMES_NUMBER;$i++){
	...
	echo '<td><center>T'.$i.'_'.$bg_name.'</center></td>';
...
for($i=0;$i<MINISCRIPTS_NUMBER;$i++){
	...
	echo '<td><center>S'.$i.'_'.$bg_name.'</center></td>';
...
if(!isset($id)) continue;
	...
	$data_e = $pdo->query("SELECT emotion_id FROM emotions_stat WHERE id = $current_id");
	while($r = $data_e->fetch(PDO::FETCH_BOTH)) {
