...
//Показва плътността на избраните състояния
	$table_result .= '<tr><td style="border: 1px solid #c0c0c0;"><a title="'.quot($script_name).', '.quot($script_desc).'">* '.quot($bg_name).'</a></td>';
...
$data_s = $pdo->query('SELECT miniscripts.id, miniscripts.bg_name, miniscripts.domain1_id, miniscripts.domain2_id, miniscripts.bg_name, miniscripts.bg_desc, miniscripts.en_name FROM miniscripts inner join miniscripts_stat on miniscripts.id = miniscripts_stat.miniscript_id WHERE miniscripts_stat.id='.$id.'');
...
$table_result .= '<tr><td colspan="2" align="left" style="border-right: none; border: 1px solid #c0c0c0;">* <b><a title="'.quot($r['en_name']).', '.quot($r['bg_desc']).'">'.$r['bg_name'].'</a></b>, ';
...
$table_result.= '<tr><th colspan="2"><center><br/><b>СЪЗДАЙ СВОЯ "КАРТА НА ЧУВСТВАТА"!<center/><form action="http://testrain.info/download/Flowscape.pdf" target="_blank" method="get"><input type="submit" value="Виж как става"></form><center/></th><tr/>';
...
$data = $pdo->query("SELECT bg_name,bg_desc FROM management WHERE score_group LIKE '%$score_group%'");
$r = $data->fetch(PDO::FETCH_BOTH);
if ($r){
	$bg_name = $r['bg_name'];
	$bg_desc = $r['bg_desc'];
...
//Demorest --> Показва процентите от "statements"
...
while($r3 = $sel_states->fetch(PDO::FETCH_BOTH)){
	...
	$table_result .= '<td style="border: 1px solid #c0c0c0;">Важност: <b>'.$s_sl.'<b/></td></tr>';
}
...
//Показва избраните изречения за всеки "axes"
	...
	$table_result.= '<br><a title="'.quot($bg_desc).'">* <b/>'.quot($bg_name).'<a title="'.$chosen_statis.'">, '.$level_axes.'%<a title="Показва в проценти каква е степента на изразеност на показателя."></b> изразеност</a></br>'."\n";
}	
$table_result.='</td><tr/>';
$table_result .= '<center/></table>';
echo $table_result;
