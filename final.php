<?php
	...
	//Показва избраните изречения за всеки "axis"
		...
		$level_axis = percent(count($axis_list[$axis_id]), $axis_total);
			...
			$table_result.= '<tr><td><a title="'.$chosen_states.'"><b> #'.quot($bg_name).'</b></a>; '.$count_axis.' from '. $axis_total.' sentences ('.$level_axis.'%);</a><br></td><td class="top"><input type="button" data-id="axis' . $axis_id . '" class="show" value="read"></td>'."</tr>";
		...
		if($c == '1') {
			...
			$table_result.='"> +'.quot($bg_name).'</a>; '.$count.' от '. $subaxis_total.' твърдения;</a><br></td>'.'<td class="top"><input type="button" data-id="subaxis' . $subaxis . '" class="show" value="read"></td>'."</tr>";
		...
		if($c == '4') {
			...
			/*foreach($axis_subaxis[$axis_id] as $subaxis => $count){
				...
				$table_result.='"> +'.quot($bg_name).'</a>; '.$count.' from '. $subaxis_total.' sentences;</a><br></td>'.'<td class="top"><input type="button" data-id="subaxis' . $subaxis . '" class="show" value="read"></td>'."</tr>";
				$table_result.='<tr><td colspan="2"><label class="desc-res2" style="display: none;" id="subaxis' . $subaxis . '">'. $bg_desc .'"</label></td></tr>';
			}*/
		}
