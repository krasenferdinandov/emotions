...
$data = $pdo->query('SELECT id,bg_name, en_name, bg_desc,domain1_id,domain2_id FROM miniscripts WHERE ( '.str_replace('X', '1', $miniscript_condition).' ) AND ( '.str_replace('X', '2', $miniscript_condition).' );');
while($r = $data->fetch(PDO::FETCH_BOTH)){
		$m=$r['id'];
//$table_result .=  $m;//Показва id на всичките възможни miniscripts;
...
//$table_result .= '<th colspan="2" align="left" style="border: 0px solid #c0c0c0;"><input id="script_'.$m.'" type="checkbox" name="miniscript'.$m.'" value="1">';
$table_result .= '<a title="'.quot($r['en_name']).','.quot($r['bg_desc']).'"><label for="script_'.$m.'">'.$r['bg_name'].'</a>
</label></th></tr>';
}
...
$table_result.= '<tr><td style="border: 1px solid #c0c0c0;">'.CONTROL.'</td>'.'<td style="border: 1px solid #c0c0c0;"><a title="'.quot($bg_desc).'">* '.quot($bg_name).'</a></br></td><tr/>';
